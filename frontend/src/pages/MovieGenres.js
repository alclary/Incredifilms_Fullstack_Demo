...
async function fetchMovieGenres() {
  const movie_genres = await axios.get("http://localhost:3001/moviegenres");
...
export default function MovieGenres() {
  ...
  async function handleDelete(row) {
    if (
      ...
      let deleted = await axios.delete(
        `http://localhost:3001/moviegenres/${row.movie_genre_id}`
      );
  ...
  return (
    <div>
      ...
      <Grid
        columns={[
          { name: "Movie Genre ID", id: "movie_genre_id", sort: true },
